...
async def analyze_symbol(
    ...
    async with AsyncSessionLocal() as db:
        ...
        try:
            return await orchestrator.analyze(request.symbol, request.question or "", current_user.id)
...
async def get_news_data(symbol: str, current_user: User = Depends(get_current_user)):
    agent = NewsAgent()
    try:
        return await agent.analyze(symbol)
    except Exception as e:
...
async def get_sentiment_data(symbol: str, current_user: User = Depends(get_current_user)):
...
async def get_risk_data(symbol: str, current_user: User = Depends(get_current_user)):
...
async def get_trade_data(symbol: str, current_user: User = Depends(get_current_user)):
    market_agent = MarketAgent()
    news_agent = NewsAgent()
    sentiment_agent = SentimentAgent()
