...
export default function MarketExplorer() {
  ...
  return <div className="cc-page cc-market-explorer">
    ...
    <section className="cc-card cc-catalog-card" aria-label="Crypto market directory">
      ...
      <div className="cc-table-scroll"><table className="cc-market-table"><thead><tr><th scope="col">Asset</th><th scope="col">Price</th><th scope="col">24h change</th><th scope="col">24h volume</th><th scope="col">Market cap</th><th scope="col">Price history</th><th scope="col"><span className="cc-sr-only">Asset details</span></th></tr></thead><tbody>{coins.map(coin => <tr key={coin.id}><td><div className="cc-inline"><CoinIcon symbol={coin.symbol} image={coin.image}/><strong>{coin.name}</strong></div></td><td>{money(coin.currentPrice)}</td><td className={coin.priceChange === null ? "cc-muted" : coin.priceChange < 0 ? "cc-negative" : "cc-positive"}>{percent(coin.priceChange)}</td><td>{money(coin.volume, true)}</td><td>{money(coin.marketCap, true)}</td><td><Sparkline prices={coin.sparkline}/></td><td><button type="button" className="cc-button cc-analyze-link" onClick={() => setSelected(coin)} aria-label={`View ${coin.name} details`}>View <ArrowUpRight size={14}/></button></td></tr>)}</tbody></table></div>
